...
class TestBaseDataDisclosureAgreementTemplate(AsyncTestCase):
    """Test base data disclosure agreement template"""

    async def setUp(self) -> None:
        self.dda = DataDisclosureAgreementModel(
        ...
        self.base_dda_template = BaseDataDisclosureAgreementTemplate(dda=self.dda)
    ...
    async def test_data_disclosure_agreement_merkletree(self, mock_nquads) -> None:
        ...
        mt = self.base_dda_template.merkle_tree

        assert isinstance(mt, MerkleTree)
        assert len(mt.leaves) == 1

    @async_mock.patch(("pyld.jsonld.normalize"))
    async def test_data_disclosure_agreement_nquads(self, mock_normalize) -> None:
        ...
        nquads = self.base_dda_template.nquads()

        assert isinstance(nquads, list)
        assert len(nquads) == 1

    @async_mock.patch(
        "dexa_sdk.agreements.dda.v1_0.templates.BaseDataDisclosureAgreementTemplate.nquads"
    )
    ...
    async def test_data_disclosure_agreement_did_mydata(
        ...
        did = self.base_dda_template.mydata_did
